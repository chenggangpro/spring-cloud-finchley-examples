...
/**
 * @classDesc: GatewayContextFilter
 * @author: chenggang
...
public class GatewayContextFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String serviceId = StringUtils.substringBetween(path, GatewayUtils.PATH_SEPARATOR,GatewayUtils.PATH_SEPARATOR);
        String realPath = StringUtils.substringAfter(path,GatewayUtils.PATH_SEPARATOR+serviceId);
        GatewayContext gatewayContext = new GatewayContext();
        gatewayContext.getAllRequestData().addAll(request.getQueryParams());
        gatewayContext.setPath(realPath);
        gatewayContext.setServiceId(serviceId);
        gatewayContext.setFullPath(path);
        /**
         * save request full path into gateway context
         */
        String uri = exchange.getRequest().getURI().toString();
        String url;
        if(StringUtils.indexOf(uri,"?")>-1){
            url = StringUtils.substringBefore(uri, "?");
        }else{
            url = uri;
        }
        gatewayContext.setFormatUrl(url);
        /**
         * save gateway context into exchange
         */
        exchange.getAttributes().put(GatewayContext.CACHE_GATEWAY_CONTEXT,gatewayContext);
        log.debug("[GatewayContext]Request ServiceId:{},Request Path:{}",serviceId,path);
    ...
    private Mono<Void> readFormData(ServerWebExchange exchange, GatewayFilterChain chain, GatewayContext gatewayContext){
        ...
        return exchange.getFormData()
                .doOnNext(multiValueMap -> {
                    ...
                    log.debug("[GatewayContext]Read FormData:{},Gateway context is set with {}",multiValueMap,exchange);
                })
                .then(Mono.defer(() -> {
                    ...
                    log.debug("[GatewayContext]Rewrite Form Data :{},Gateway context is set with {}",formDataBodyString,gatewayContext);
                ...
                .flatMap(dataBuffer -> {
                    ...
                    return ServerRequest.create(mutatedExchange, messageReaders)
                            ...
                            .doOnNext(objectValue -> {
                                ...
                                log.debug("[GatewayContext]Read JsonBody:{},Gateway context is set with {}",objectValue,gatewayContext);
